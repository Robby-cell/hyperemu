...
fn test_x86_alu_add_flags() {
    ...
    cpu.regs[REG_EAX] = 10;
    cpu.regs[REG_EBX] = 20;
    execute_instr(&mut cpu, instr.clone(), &mut bus).unwrap();
...
fn test_x86_parity_flag() {
    ...
    execute_instr(&mut cpu, instr2, &mut bus).unwrap();
    let f = EFlags::from_bits_retain(cpu.regs[REG_EFLAGS]);
...
fn test_x86_stack_ops() {
    ...
    // PUSH eax
    execute_instr(&mut cpu, Instr::Push(Operand::Reg(GpReg::Eax)), &mut bus).unwrap();
    assert_eq!(cpu.regs[REG_ESP], 0x1EFC);
    assert_eq!(bus.read_32(0x1EFC).unwrap(), 0x12345678);

    // POP ebx
    execute_instr(&mut cpu, Instr::Pop(Operand::Reg(GpReg::Ebx)), &mut bus).unwrap();
...
fn test_x86_call_ret() {
    ...
    execute_instr(&mut cpu, Instr::Call(0x50), &mut bus).unwrap();
    ...
    // RET
    execute_instr(&mut cpu, Instr::Ret, &mut bus).unwrap();
...
fn test_x86_enum_adc_sbb_carry_chain() {
    ...
    execute_instr(&mut cpu, add_low, &mut bus).unwrap();
    ...
    execute_instr(&mut cpu, adc_high, &mut bus).unwrap();
...
fn test_x86_enum_memory_displacements() {
    ...
    execute_instr(&mut cpu, instr_store, &mut bus).unwrap();
    ...
    execute_instr(&mut cpu, instr_load, &mut bus).unwrap();
    assert_eq!(
